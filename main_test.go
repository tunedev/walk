	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T){
			...
			if len(lines) != expLogLines {
					t.Errorf("Expecred %d log lines, got %d instead\n",
...
func TestRunArchive(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T){
			...
			actLen := len(filesArchived) 
			if actLen != tc.nArchive {
				t.Errorf("Expected %d files to be archived, got %d instead\n", tc.nArchive, actLen)
			}
		})
	}
}
